...
const RoleTabs = ({
  ...
  githubUsername,
  image,
}: { roles: Role[]; githubUsername: string | undefined; image: string | undefined }) => {
          ...
          <div className="flex gap-4 py-6 md:py-10 min-w-max">
            {roles.map((role) => (
              <button
                ...
                className={`px-3 md:px-4 py-2 text-lg md:text-2xl font-mokoto transition-colors duration-500 hover:text-textyellow ${activeRole === role ? "text-textyellow border-b-2 border-textyellow" : "text-white"
                  }`}
              >
                {role.toUpperCase()}
              </button>
